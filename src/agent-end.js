...
function getErrorText(lastMsg, event) {
  const candidates = [lastMsg?.errorMessage, lastMsg?.content, lastMsg?.message]
  ...
  const anyMsg = event?.messages?.find((message) => {
    const text = extractText(
      message?.errorMessage || message?.content || message?.message,
    )
...
const isGsdExtension = (extPath) =>
  extPath.includes('extensions') &&
  (extPath.includes('/gsd/') || extPath.endsWith('/gsd'))
...
export function createAgentEndHandler(pi) {
  const handler = async (event, ctx) => {
    ...
    if (state.repairExhaustedThisTurn) {
      ...
      ctx.ui?.notify?.(
        '[Guardian] Repair exhausted. Returning control.',
        'error',
      )
      return
    }
    ...
    if (state.retryCount <= RETRY_MAX) {
      ...
      ctx.ui?.notify?.(
        `[Guardian] Error: ${errorText.slice(0, 150)}...`,
        'error',
      )
      ctx.ui?.notify?.(
      ...
      // State Claim Contract: declare recovery intent so orchestrators (e.g. DAG)
      // can detect that recovery is in progress without hardcoded timeouts.
      if (ctx.session) ctx.session.isRecovering = true
      try {
        await sleep(pi, delayMs)
      } catch {
        if (ctx.session) ctx.session.isRecovering = false
      ...
      // Release recovery claim. The retry prompt (sent above) should cause
      // session.isStreaming to become true; the orchestrator will pick it up.
      if (ctx.session) ctx.session.isRecovering = false
      return
    }

    // Budget exhausted in sub-session: don't start interactive repair flow.
    // DAG will detect isRecovering=false (never set for this path) and fail the task itself.
    if (ctx.session?.isSubSession) {
      ctx.ui?.notify?.(
        '[Guardian] Retry budget exhausted in sub-session. Handing back to orchestrator.',
        'warning',
      )
      return
    }

    // Budget exhausted on main session: hand over to interactive repair flow
